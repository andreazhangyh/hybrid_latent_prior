...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        self.vq_objective_weight = vq_objective_weight
                ...
                if self.commitment_use_cross_entropy_loss:
                    ...
                    if exists(mask):
                        ...
                        if not self.learnable_codebook:
                            ...
                            commit_loss = F.mse_loss(commit_quantize.detach(), x, reduction = 'none')
                        ...
                        commit_loss = commit_loss[loss_mask].sum(dim=-1).mean()
                    else:
                        if not self.learnable_codebook:
                            # commit_loss = F.mse_loss(commit_quantize, x)
                            commit_loss = F.mse_loss(commit_quantize, x, reduction = 'none').sum(dim=-1).mean()
                        else:
                            # commit_loss = F.mse_loss(commit_quantize.detach(), x)
                            commit_loss = F.mse_loss(commit_quantize.detach(), x, reduction = 'none').sum(dim=-1).mean()

                loss = loss + commit_loss * self.commitment_weight

                if self.learnable_codebook:
                    # vq_objective = F.mse_loss(commit_quantize, x.detach())
                    vq_objective = F.mse_loss(commit_quantize, x.detach(), reduction = 'none').sum(dim=-1).mean()
                    loss = loss + vq_objective * self.vq_objective_weight
