...
if __name__ == "__main__":
    ...
    save_root = os.environ.get("LAFAN_NPZ_OUT_DIR", os.path.join("data", "LAFAN", "lafan1_npz" + date_str))
    split_actors = {
        "train": ['subject1', 'subject2', 'subject3', 'subject4'],
        "test": ['subject5'],
    }

    for split, actors in split_actors.items():
        save_dir = os.path.join(save_root, split)
        os.makedirs(save_dir, exist_ok=True)

        for bvh_file in tqdm(bvh_files, desc=split):
